...
echo "Portação OMNeT++ para NS3 - Versão Final Funcional"
echo ""

# Configurações
NS3_DIR="/home/lipef/edtest/ns-allinone-3.40/ns-3.40"
RESULTS_DIR="/home/lipef/edtest/omnet-to-ns3-port/results/csv"
SIMULATION_FILE="tdeo-final-working"

# Potências conforme documento OMNeT++
POWERS=(2 5 10 15)
...
rm -f "$RESULTS_DIR/tdeo-final-working.csv"
...
for power in "${POWERS[@]}"; do
    ...
    ./ns3 run "$SIMULATION_FILE --txPower=$power --simTime=600 --results=$RESULTS_DIR/tdeo-final-working.csv"
...
echo "Arquivo de resultados: $RESULTS_DIR/tdeo-final-working.csv"
echo ""

# Mostrar resumo dos resultados
if [ -f "$RESULTS_DIR/tdeo-final-working.csv" ]; then
    ...
    for power in "${POWERS[@]}"; do
        ...
        success_nodes=$(grep "^$power," "$RESULTS_DIR/tdeo-final-working.csv" | awk -F',' '$5 > 0' | wc -l)
        
        # Calcular taxa média de sucesso
        avg_success=$(grep "^$power," "$RESULTS_DIR/tdeo-final-working.csv" | awk -F',' '{sum+=$5; count++} END {if(count>0) printf "%.2f", sum/count; else print "0.00"}')
...
echo "python3 /home/lipef/edtest/omnet-to-ns3-port/scripts/analyze-results.py $RESULTS_DIR/tdeo-final-working.csv"
echo ""
